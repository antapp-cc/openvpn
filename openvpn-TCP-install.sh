...
# Function to download and generate China IP routes
generate_china_routes() {
    ...
    if [[ "$downloaded" -eq 0 ]]; then
        echo "Error: Failed to download China IP list. Falling back to global proxy."
        return 1
    fi

    echo "Generating routing rules, please wait..."
    ...
    rm -f "$cn_temp_file"
    echo "China bypass routing rules added successfully."
    return 0
}
...
server 10.8.0.0 255.255.255.0" > /etc/openvpn/server/server.conf
	...
	case "$routing" in
		1|"")
			...
			;;
		2|3)
			# Split tunneling modes don't push redirect-gateway
			;;
	esac
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	case "$routing" in
		2)
			# Split tunnel - manual routes needed
			echo "
...
" >> /etc/openvpn/server/client-common.txt
		...
		3)
			# China bypass
			echo "max-routes 10000
route-nopull
# Default route through VPN
route 0.0.0.0 128.0.0.0 vpn_gateway
route 128.0.0.0 128.0.0.0 vpn_gateway" >> /etc/openvpn/server/client-common.txt
			
			# Generate China IP routes
			if ! generate_china_routes; then
				# Fallback to global proxy if download fails
				sed -i '/max-routes/d; /route-nopull/d; /route 0.0.0.0/d; /route 128.0.0.0/d; /China direct routes/d' /etc/openvpn/server/client-common.txt
				if [[ -z "$ip6" ]]; then
					echo 'push "redirect-gateway def1 bypass-dhcp"' >> /etc/openvpn/server/server.conf
				else
					echo 'push "redirect-gateway def1 ipv6 bypass-dhcp"' >> /etc/openvpn/server/server.conf
